using System;
using Microsoft.EntityFrameworkCore.Migrations;

namespace Infrastructure.EF.Migrations
{
    public partial class v1 : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "User",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    FirstName = table.Column<string>(type: "nvarchar(max)", nullable: false),
                    LastName = table.Column<string>(type: "nvarchar(max)", nullable: false),
                    Email = table.Column<string>(type: "nvarchar(450)", nullable: false),
                    Password = table.Column<string>(type: "nvarchar(max)", nullable: false),
                    CreatedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    IsActive = table.Column<bool>(type: "bit", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_User", x => x.Id);
                });

            migrationBuilder.CreateTable(
                name: "Post",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    Title = table.Column<string>(type: "nvarchar(max)", nullable: false),
                    Description = table.Column<string>(type: "nvarchar(max)", nullable: true),
                    UploadedUserId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CreatedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    IsActive = table.Column<bool>(type: "bit", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_Post", x => x.Id);
                    table.ForeignKey(
                        name: "FK_Post_User_UploadedUserId",
                        column: x => x.UploadedUserId,
                        principalTable: "User",
                        principalColumn: "EntityId");
                });

            migrationBuilder.CreateTable(
                name: "Comment",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    Description = table.Column<string>(type: "nvarchar(max)", nullable: false),
                    UploadedUserId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    PostId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CreatedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    IsActive = table.Column<bool>(type: "bit", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_Comment", x => x.Id);
                    table.ForeignKey(
                        name: "FK_Comment_Post_PostId",
                        column: x => x.PostId,
                        principalTable: "Post",
                        principalColumn: "EntityId",
                        onDelete: ReferentialAction.Cascade);
                    table.ForeignKey(
                        name: "FK_Comment_User_UploadedUserId",
                        column: x => x.UploadedUserId,
                        principalTable: "User",
                        principalColumn: "EntityId");
                });

            migrationBuilder.CreateTable(
                name: "UserDislikedPost",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    UserId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    PostId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CreatedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    IsActive = table.Column<bool>(type: "bit", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_UserDislikedPost", x => x.Id);
                    table.ForeignKey(
                        name: "FK_UserDislikedPost_Post_PostId",
                        column: x => x.PostId,
                        principalTable: "Post",
                        principalColumn: "EntityId");
                    table.ForeignKey(
                        name: "FK_UserDislikedPost_User_UserId",
                        column: x => x.UserId,
                        principalTable: "User",
                        principalColumn: "EntityId");
                });

            migrationBuilder.CreateTable(
                name: "UserLikedPost",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    UserId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    PostId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CreatedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    IsActive = table.Column<bool>(type: "bit", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_UserLikedPost", x => x.Id);
                    table.ForeignKey(
                        name: "FK_UserLikedPost_Post_PostId",
                        column: x => x.PostId,
                        principalTable: "Post",
                        principalColumn: "EntityId");
                    table.ForeignKey(
                        name: "FK_UserLikedPost_User_UserId",
                        column: x => x.UserId,
                        principalTable: "User",
                        principalColumn: "EntityId");
                });

            migrationBuilder.CreateTable(
                name: "UserViewedPost",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    UserId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    PostId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CreatedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    IsActive = table.Column<bool>(type: "bit", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_UserViewedPost", x => x.Id);
                    table.ForeignKey(
                        name: "FK_UserViewedPost_Post_PostId",
                        column: x => x.PostId,
                        principalTable: "Post",
                        principalColumn: "EntityId");
                    table.ForeignKey(
                        name: "FK_UserViewedPost_User_UserId",
                        column: x => x.UserId,
                        principalTable: "User",
                        principalColumn: "EntityId");
                });

            migrationBuilder.CreateTable(
                name: "UserDislikedComment",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    UserId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CommentId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CreatedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    IsActive = table.Column<bool>(type: "bit", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_UserDislikedComment", x => x.Id);
                    table.ForeignKey(
                        name: "FK_UserDislikedComment_Comment_CommentId",
                        column: x => x.CommentId,
                        principalTable: "Comment",
                        principalColumn: "EntityId");
                    table.ForeignKey(
                        name: "FK_UserDislikedComment_User_UserId",
                        column: x => x.UserId,
                        principalTable: "User",
                        principalColumn: "EntityId");
                });

            migrationBuilder.CreateTable(
                name: "UserLikedComment",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    UserId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CommentId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    CreatedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    ModifiedAt = table.Column<DateTime>(type: "datetime2", nullable: false),
                    IsActive = table.Column<bool>(type: "bit", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_UserLikedComment", x => x.Id);
                    table.ForeignKey(
                        name: "FK_UserLikedComment_Comment_CommentId",
                        column: x => x.CommentId,
                        principalTable: "Comment",
                        principalColumn: "EntityId");
                    table.ForeignKey(
                        name: "FK_UserLikedComment_User_UserId",
                        column: x => x.UserId,
                        principalTable: "User",
                        principalColumn: "EntityId");
                });

            migrationBuilder.CreateIndex(
                name: "IX_Comment_PostId",
                table: "Comment",
                column: "PostId");

            migrationBuilder.CreateIndex(
                name: "IX_Comment_UploadedUserId",
                table: "Comment",
                column: "UploadedUserId");

            migrationBuilder.CreateIndex(
                name: "IX_Post_UploadedUserId",
                table: "Post",
                column: "UploadedUserId");

            migrationBuilder.CreateIndex(
                name: "IX_User_Email",
                table: "User",
                column: "Email");

            migrationBuilder.CreateIndex(
                name: "IX_UserDislikedComment_CommentId",
                table: "UserDislikedComment",
                column: "CommentId");

            migrationBuilder.CreateIndex(
                name: "IX_UserDislikedComment_UserId",
                table: "UserDislikedComment",
                column: "UserId");

            migrationBuilder.CreateIndex(
                name: "IX_UserDislikedPost_PostId",
                table: "UserDislikedPost",
                column: "PostId");

            migrationBuilder.CreateIndex(
                name: "IX_UserDislikedPost_UserId",
                table: "UserDislikedPost",
                column: "UserId");

            migrationBuilder.CreateIndex(
                name: "IX_UserLikedComment_CommentId",
                table: "UserLikedComment",
                column: "CommentId");

            migrationBuilder.CreateIndex(
                name: "IX_UserLikedComment_UserId",
                table: "UserLikedComment",
                column: "UserId");

            migrationBuilder.CreateIndex(
                name: "IX_UserLikedPost_PostId",
                table: "UserLikedPost",
                column: "PostId");

            migrationBuilder.CreateIndex(
                name: "IX_UserLikedPost_UserId",
                table: "UserLikedPost",
                column: "UserId");

            migrationBuilder.CreateIndex(
                name: "IX_UserViewedPost_PostId",
                table: "UserViewedPost",
                column: "PostId");

            migrationBuilder.CreateIndex(
                name: "IX_UserViewedPost_UserId",
                table: "UserViewedPost",
                column: "UserId");
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "UserDislikedComment");

            migrationBuilder.DropTable(
                name: "UserDislikedPost");

            migrationBuilder.DropTable(
                name: "UserLikedComment");

            migrationBuilder.DropTable(
                name: "UserLikedPost");

            migrationBuilder.DropTable(
                name: "UserViewedPost");

            migrationBuilder.DropTable(
                name: "Comment");

            migrationBuilder.DropTable(
                name: "Post");

            migrationBuilder.DropTable(
                name: "User");
        }
    }
}
